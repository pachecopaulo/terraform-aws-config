...
# In order to update the aws-load-balancer-controller version
# the iam_policy.json definition also needs to be updated 
# otherwise ingress could fail due to permission issues
resource "helm_release" "using_iamserviceaccount" {
  ...
  # Number of controller pods to run, only one will be active due to leader election
  set {
    name  = "replicaCount"
    value = 2
  }
